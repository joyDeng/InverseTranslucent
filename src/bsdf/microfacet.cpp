...
SpectrumC Microfacet::eval(const IntersectionC& its, const BSDFSampleC& bs, MaskC active) const {
    // Vector3fC wo = its.sh_frame.to_world(bs.wo);
    return __eval<false>(its, bs.wo, active);
}


SpectrumD Microfacet::eval(const IntersectionD& its, const BSDFSampleD& bs, MaskD active) const {
    // Vector3fD wo = its.sh_frame.to_world(bs.wo);
    return __eval<true>(its, bs.wo, active);
}

FloatC Microfacet::pdf(const IntersectionC& its, const BSDFSampleC& bs, MaskC active) const {
    return __pdf<false>(its, bs.wo, active);
}


FloatD Microfacet::pdf(const IntersectionD& its, const BSDFSampleD& bs, MaskD active) const {
...
Spectrum<ad> Microfacet::__eval(const Intersection<ad> &its, const Vector3f<ad> &wo, Mask<ad> active) const {
    ...
    masked(roughness, roughness < 0.089f) = 0.089f;  // clamp to >= 0.089
...
BSDFSampleC Microfacet::sample(const Scene *scene, const IntersectionC& its, const Vector8fC& sample, MaskC active) const {
    return __sample<false>(its, sample, active);
}


BSDFSampleD Microfacet::sample(const Scene *scene, const IntersectionD& its, const Vector8fD& sample, MaskD active) const {
...
Float<ad> Microfacet::__pdf(const Intersection<ad>& its, const Vector3f<ad>& wo, Mask<ad> active) const {
    ...
    Float<ad> alpha_u = sqr(m_roughness.eval<ad>(its.uv));
    Float<ad> alpha_v = sqr(m_roughness.eval<ad>(its.uv));
...
template <bool ad>
BSDFSample<ad> Microfacet::__sample(const Intersection<ad>& its, const Vector8f<ad>& sample, Mask<ad> active) const {
    BSDFSample<ad> bs;
    Float<ad> cos_theta_i = Frame<ad>::cos_theta(its.wi);
    Float<ad> alpha_u = sqr(m_roughness.eval<ad>(its.uv));
    Float<ad> alpha_v = sqr(m_roughness.eval<ad>(its.uv));
