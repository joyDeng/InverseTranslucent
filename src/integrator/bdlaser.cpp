...
#include <psdr/integrator/bdlaser.h>
...
BDLaserIntegrator::~BDLaserIntegrator() {
...
BDLaserIntegrator::BDLaserIntegrator(int bsdf_samples, int light_samples) : m_bsdf_samples(bsdf_samples), m_light_samples(light_samples) {
    PSDR_ASSERT((bsdf_samples >= 0) && (light_samples >= 0) && (bsdf_samples + light_samples > 0));
}


SpectrumC BDLaserIntegrator::Li(const Scene &scene, Sampler &sampler, const RayC &ray, MaskC active) const {
    return __Li<false>(scene, sampler, ray, active);
}


SpectrumD BDLaserIntegrator::Li(const Scene &scene, Sampler &sampler, const RayD &ray, MaskD active) const {
    return __Li<true>(scene, sampler, ray, active);
}


template <bool ad>
Spectrum<ad> BDLaserIntegrator::__Li(const Scene &scene, Sampler &sampler, const Ray<ad> &ray, Mask<ad> active) const {
    ...
    // std::cout<<"[Laser__Li]:test3"<<std::endl;
    Spectrum<ad> bsdf_val = bsdf_array->eval(its, bs, active);
    // std::cout<<"[Laser__Li]:test4"<<std::endl;
    Float<ad> distance = norm(bs.po.p - lits.p);
    ...
    masked(result, active) = Ei * bsdf_val / bs.pdf * warp::square_to_cosine_hemisphere_pdf<ad>(bs.wo) / area;
    // std::cout<<"[Laser__Li]:test6"<<std::endl;
    return result;
}


void BDLaserIntegrator::preprocess_secondary_edges(const Scene &scene, int sensor_id, const ScalarVector4i &reso, int nrounds) {
...
void BDLaserIntegrator::render_secondary_edges(const Scene &scene, int sensor_id, SpectrumD &result) const {
...
template <bool ad>
std::pair<IntC, Spectrum<ad>> BDLaserIntegrator::eval_secondary_edge(const Scene &scene, const Sensor &sensor, const Vector3fC &sample3) const {
