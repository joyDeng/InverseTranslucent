...
PSDR_CLASS_DECL_BEGIN(ColocateIntegrator, final, Integrator)
public:
    ColocateIntegrator(int bsdf_samples = 1, int light_samples = 1);
    virtual ~ColocateIntegrator();
...
protected:
    ...
    template <bool ad>
    Spectrum<ad> __Li(const Scene &scene, Sampler &sampler, const Ray<ad> &ray, Mask<ad> active, int sensor_id) const;
...
PSDR_CLASS_DECL_END(ColocateIntegrator)
